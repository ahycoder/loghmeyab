...
public class ActivityLogin extends AppCompatActivity{
    ...
    protected void onResume() {
        ...
        if(state_login== Constant.STATE_GET_MOBILE){
            stateUiGetMobile();
        }else if(state_login== Constant.STATE_SEND_CODE){
            stateUiSendCode();
            if(bundle !=null) {
              countDownTimer(bundle.getInt("RemindTime"));
            }
        }else if(state_login== Constant.STATE_FAILD_CODE){
            stateUiFailedCode();
            if(bundle !=null) {
                countDownTimer(bundle.getInt("RemindTime"));
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(stateLogin== Constant.STATE_GET_MOBILE){
                    getMobileAndSendToServer();
                }else if(stateLogin== Constant.STATE_SEND_CODE){
                    sendVrifyCode();
                } else if(stateLogin== Constant.STATE_FAILD_CODE){
                    sendVrifyCode();
                }
            }
        });
              ...
              public void onResponse(JSONArray response) {
                  ...
                  try {
                      if (response.get(0).toString().equals(300+"")){
                          ...
                          stateUiFailedCode();
                          sEdit.putInt(Constant.STATE_LOGIN,Constant.STATE_FAILD_CODE);
                      }else if (response.get(0).toString().equals(200+"")){
                          countDownTimer.cancel();
                          sEdit.putInt(Constant.STATE_LOGIN,Constant.STATE_LOGIN_OK);
...
//                          }
    ...
    private void getMobileAndSendToServer() {
        ...
        if(isMobile(edtLogin.getText().toString())){
                  ...
                  public void onResponse(JSONArray response) {
                      ...
                      try {
                          if (response.get(0).toString().equals(400+"")){
                              Toast.makeText(G.context,response.get(1).toString(),Toast.LENGTH_SHORT).show();
                          }else if (response.get(0).toString().equals(200+"")) {
                              Log.i("TAG","response: "+response);
                              SharedPreferences.Editor sEdit = G.preferences.edit();
                              sEdit.putInt(Constant.STATE_LOGIN,Constant.STATE_SEND_CODE);
            ...
            Toast.makeText(G.context,"شماره همراه را درست وارد کنید",Toast.LENGTH_SHORT).show();
        }
